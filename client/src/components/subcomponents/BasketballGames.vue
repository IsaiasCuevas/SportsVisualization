<template>
  <div>
    <h1>Basketball 2018 Season Results</h1>
    <div class="container">
      ...
      <div class="mcontainer">
          <!-- Slice method to return selected objects in array as a new array object-->
        <div class="card" v-bind:key="index + '1'" v-for="(i, index) in games.games.slice(0,selected)">
          <h4 style>
            {{games.games[index].home.name}}
            <br />vs
            <br />
            {{games.games[index].away.name}}
          </h4>
          <hr />
          <div
            style="  justify-content: center;
                    align-items: center; font-size: 22px;"
          >({{games.games[index].home_points}} - {{games.games[index].away_points}})</div>
        ...
        <basketballStats v-bind:stats="id">Hi</basketballStats>
      </div>
    </div>
  </div>
</template>
...
<script>
import games from '../../BasketBallData/Game_Schedule/BB_Schedule.json'
import basketballStats from "../modals/basketballStats";
