        ...
        <stats-card data-background-color="purple">
          ...
          <template slot="content">
            <p class="category" v-for="(i, index) in stat.home.leaders.assists" :key="'0' + index">{{i.full_name}} ({{i.position}})</p>
             <h3 class="title" v-for="(i, index) in stat.home.leaders.assists" :key="'0' + index"> AST {{i.statistics.points}}</h3>
          </template>
        ...
        <stats-card data-background-color="purple">
          ...
          <template slot="content">
            <p class="category" v-for="(i, index) in stat.away.leaders.assists" :key="'0' + index">{{i.full_name}} ({{i.position}})</p>
             <h3 class="title" v-for="(i, index) in stat.away.leaders.assists" :key="'0' + index"> AST {{i.statistics.points}}</h3>
          </template>
...
async created() {
    try {
      const res = await axios.get(`http://localhost:8080/Game_Boxscore/` + this.stats.id + `.json`);
      const res2 = await axios.get(`http://localhost:8080/Game_Summary/` + this.stats.id + `.json`);
      this.stat = res.data;
  ...
  methods: {
    ...
    onSelect3(i) {
      ...
      this.options = {
        labels: [this.selected2.name, this.selected3.name],
        title: {
          text: "Steals"
        }
      }
